    ...
    <div class="container bg-light rounded border border-primary">
        <div class="row justify-content-center">
            <div class="col text-center w-100 p-3 rounded border border-secondary">
                @if (Route::has('route_missions_applications_teacher_accept'))
                    <form method="post" action="/missions/applications_teacher/accept">
                        @csrf
                        @if($mxfault == true)
                            <div class="container bg-warning text-white rounded">
                                <div class="row justify-content-center">
                                    <div class="col text-center">
                                        <p>Заявка уже существует</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                ...
                @if (Route::has('route_missions_missions_applications_teacher'))
                    <div class="col-sm">
                        <form method="get" action="/missions/missions_applications_teacher">
                            @csrf
                            <button type="submit" class="btn btn-danger w-100"><h7>Отмена</h7></button>
                        </form>
                    </div>
                @endif
            </div>
